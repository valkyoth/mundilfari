...
| Hard-bound assumptions are lost or substituted | immutable bounded content-addressed assumption sets; conjunction through intersection/consensus; preservation through expansion/conversion; typed incompatibility, generation, collision, and capacity failures; no caller-selected aliases |
...
| Retrieval metadata is mistaken for verified evidence | platform/custom adapters emit only untrusted `RetrievalClaim`; engine-private verification checks registered provider generation, digest, attestation/signature, freshness, capability, and authority before opaque `VerifiedArtifactEvidence`; configured platform trust is not called cryptographic proof |
