...
| Failed members are counted as votes or filtered to weaken the quorum | complete witness preserves original membership/`n`; only current accepted-bound members contribute intervals/votes, every other status remains a non-contributor without threshold reduction, duplicate roots require correlation/diversity admission, and shortage is insufficient/unsafe until an atomic new membership generation is reassessed |
| Aborted refresh clears valid prior state or preserves invalid prior authority | outcome reports retained/invalidated/absent prior authority; operational abort retains only independently current prior state without extending it, genuine invalidation propagates despite replacement failure, and complete-new commit plus prior retirement share one linearization point |
