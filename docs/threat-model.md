...
| Dropping an unverified source arena invalidates or corrupts an accepted token | successful engine promotion creates lifetime-independent proof/token state containing canonical identities and invalidation generations; drop before completion creates nothing, while later evidence/policy/lifecycle expiry still revokes |
