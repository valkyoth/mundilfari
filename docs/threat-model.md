...
| Early claim transformation discards material needed for verification | every root and transforming interval/era/fraction/scale/UTC/POSIX/uncertainty/observation operation automatically preserves a bounded acyclic non-authoritative recipe with exact inputs/outputs/operation/rounding/model/condition/origin |
...
| Scalar/coarse monotonic read understates deadline expiry | issuance resamples after evaluator work and every strict read compares a bounded sample's conservative upper edge including resolution, sample/return latency, rate uncertainty, and completion margin; straddling or unbounded reads fail closed |
