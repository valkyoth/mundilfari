...
| Batch membership state or unused alternatives are mistaken for aggregate/consensus authority | distinct batch-state, consensus, and published authority identities; consensus binds the exact canonical bounded proof-support set while complete membership/`n` remains separate; loss of any used contributor invalidates the old authority even when unused members could form another quorum, which requires a fresh decision and identity |
| Aggregate authority compares unrelated monotonic deadlines or outlives a dependency | one explicit authority domain and conservative minimum of every required deadline; mixed domains reject or translate only through current admitted correlations whose identity/generation/uncertainty/expiry become dependencies; raw numeric comparison, omitted vector entries, and arbitrary-member deadlines fail closed |
| Aborted refresh clears valid prior state or preserves/overstates invalid prior authority | fixed-size prior observation reports retained/invalidated/absent for a tagged batch/consensus/publication subject using measured linearization coverage or typed unavailability, prior identity/generation, unchanged deadline or invalidation generation/reason, and engine/publication generation; a serialized pre-commit read plus reviewed remaining-work bound covers all steps through linearization, while missing capability/sample/domain can never retain or mint authority; operational abort retains only independently current prior state without extending it, genuine invalidation propagates despite replacement failure, complete-new commit plus prior retirement share that point, and no report claims authority through receipt |
