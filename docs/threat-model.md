...
| Failed or unprocessed members are counted as votes or filtered to weaken the quorum | complete witness preserves original membership/`n`; processed-only `CompleteMemberStatus` permits only current accepted-bound members to contribute, while disjoint `AbortMemberDiagnostic`/`Unprocessed` cannot construct a witness or invoke quorum; other processed statuses do not reduce the threshold, duplicate roots require correlation/diversity admission, and shortage is insufficient/unsafe until an atomic new membership generation is reassessed |
| Batch admission state or unused alternatives are mistaken for consensus authority | `BatchAdmissionState` has no time claim and cannot enter servo/discipline/publication/trusted-time APIs; only consensus selects a proof rule and exact canonical bounded support set while complete membership/`n` remains separate; loss of any used contributor invalidates the old authority even when unused members could form another quorum, which requires a fresh decision and identity |
| Aggregate authority compares unrelated monotonic deadlines or trusts a forged/cyclic correlation proof | one explicit authority domain and conservative minimum of every required deadline; core/platform expose directed untrusted candidates whose offset/rate/drift are hard-bound claims with complete bounded recipes, conditions, immutable paired capture anchors, and outward uncertainty growth; engine alone verifies the numerical derivations and snapshot-consistently assesses their conditions while retaining independent evidence-origin/integrity/authority/lineage/transitive-support axes; source and target validity deadlines are independently checked in their own domains and cannot be justified through the correlation; initially, every transitive recipe/condition/assessment/support leaf is forbidden from depending on a current, stale, replaced, or historical admitted correlation or a claim derived through one, making numerical proof support acyclic by construction; translation rounds outward and uses the target earliest deadline edge, and reset/suspend/rate/migration/provider/evidence/assessment/anchor/endpoint-validity loss withdraws dependents; raw numeric comparison, registration-only evidence, forged narrow bounds, two-node/longer/replacement/restored-reference cycles, circular validity, reverse/implicit composition, provider self-admission, omitted dependencies, and arbitrary-member deadlines fail closed |
| Hosted refresh is disabled without WCET, guard cleanup exhausts, or an in-progress/delayed update becomes current | portable `LinearizationRefresh` uses an engine-only RAII `RefreshReservationGuard` binding owner and execution generations, captured invalidation watermark, and nonwrapping fencing generation; the exact-domain sample is its logical point, every dependency mutation advances the watermark before exposure, and cancel/unwind/panic/early-return/drop performs a bounded allocation-free/callback-free/nonblocking/non-panicking in-place `Live`→`Tombstone` transition in the already-owned slot; higher-fence `SupersededTombstone` cleanup is a successful no-op, impossible transitions latch an engine fault and disable install, and full capacity can reject acquisition but never cleanup; readers boundedly return current state, `RefreshInProgress`, or typed fault, installed historical state is independently revalidated after unbounded preemption, and delayed writers cannot overwrite a newer invalidation/version; only tombstoned or execution-invalidated owners may be superseded by a higher fence, never by timeout alone, stale writers return `SupersededNoInstall`, restart invalidates inherited reservations, and bounded tombstone reclamation waits behind the reader generation floor; reclamation/fence exhaustion fails closed; optional `CommitCoveredRefresh` alone requires remaining-work capability |
| Async refresh suspends while owning a reservation or performs fallible/external work after acquisition | all pending, callback, and fallible work completes before acquiring `RefreshReservationGuard`; the acquiring poll returns `Ready` without another callback or `Poll::Pending`, async state never stores the live guard across polls, and guard drop during panic/unwind/cancellation tombstones the reservation |
| Aborted refresh clears valid prior state or preserves/overstates invalid prior authority | fixed-size prior-state observation reports retained/invalidated/absent for a tagged admission/consensus/publication subject using measured profile evidence or typed unavailability, prior identity/generation, unchanged deadline or invalidation generation/reason, and engine/publication generation; sample/domain/reservation failure can never retain or install new admission/authority state but ordered diagnostic invalidation remains permitted, operational abort retains only independently current prior state without extending it, genuine invalidation propagates despite replacement failure, complete-new/prior-retirement ordering is versioned, and no report claims authority through receipt |
