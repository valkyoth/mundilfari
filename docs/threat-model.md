...
| Early claim transformation discards material needed for verification | every hard claim contains a typed store/generation/domain-bound handle into a bounded canonical derivation DAG; root and transforming interval/era/fraction/scale/UTC/POSIX/uncertainty/observation operations fail unless the complete recipe is stored |
| Recipe handle is dropped, reused, or confused across stores/domains | mandatory private handles, caller-owned/fallible bounded arenas, generation rotation, typed heterogeneous edges, canonical import/reinterning, and geometry-only API exclusion fail closed |
