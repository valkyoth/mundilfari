...
| Aggregate authority compares unrelated monotonic deadlines or trusts a forged correlation | one explicit authority domain and conservative minimum of every required deadline; core/platform expose directed untrusted candidates, engine alone admits them after provider/domain/method/uncertainty/validity/lifecycle checks, translation rounds outward and uses the target earliest deadline edge, and reset/suspend/rate/migration/provider loss withdraws dependents; raw numeric comparison, reverse/implicit composition, provider self-admission, omitted dependencies, and arbitrary-member deadlines fail closed |
| Hosted refresh is disabled without WCET or an in-progress/delayed update becomes current | portable `LinearizationRefresh` uses a nonwrapping reservation/version and exact-domain sample as its logical point; readers boundedly retry or return `RefreshInProgress`, invalidation watermark/version prevents delayed overwrite, and installed historical state is independently revalidated by strict readers even after unbounded preemption; optional `CommitCoveredRefresh` alone requires remaining-work capability |
