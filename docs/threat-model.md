...
| Configured adversary assumption masquerades as evidence | every atom preserves typed measured/cryptographic/authority/configured/derived support basis through policy acceptance, estimates, and facade diagnostics |
...
| Cached synchronized label survives its deadline | every strict read samples the deadline's exact monotonic domain and fails closed at/after expiry, after reset/domain failure, or when suspend cannot be covered or invalidated, without relying on a writer or timer |
