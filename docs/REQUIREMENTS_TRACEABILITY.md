...
| exact interval and hard/statistical semantics | one early canonical identity profile binds all claim/recipe/condition/model/origin preimages and structurally handles collisions; included/excluded/unbounded algebra has no quantum adjustment; finite trusted estimates; containment claims do not imply source honesty; every hard claim has a mandatory typed handle into a bounded canonical derivation arena and external/persisted recipes remain complete unverified records; engine verification prevents unjustified narrowing; structured atom basis keeps origin, integrity, authority, and transitive derivation axes distinct; snapshot-consistent runtime assessment and policy acceptance are required for current trusted authority and propagate invalidation | identity `v0.6.1`; claims/recipes `v0.7.1`–`v0.15.0`; schema/persistence/builders `v0.22.1`, `v0.39.1`, `v0.140.0`–`v0.140.1`; engine `v0.60.0`–`v0.61.0`; downstream `v0.133.0`–`v0.138.0` |
