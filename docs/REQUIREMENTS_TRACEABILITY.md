...
| exact interval and hard/statistical semantics | one early canonical identity profile binds all claim/recipe/condition/model/origin preimages and structurally handles collisions; included/excluded/unbounded algebra has no quantum adjustment; finite trusted estimates; every hard claim has a generative lifetime-branded handle into a bounded arena with nonwrapping generations, read-lease/frozen traversal, explicit geometry/claim/fallible-derivation comparisons, and no arena-dependent infallible semantic `Eq`/`Hash`; external/persisted recipes remain complete unverified records; engine verification prevents unjustified narrowing; structured atom basis and snapshot-consistent policy acceptance propagate invalidation | identity `v0.6.1`; claims/recipes `v0.7.1`–`v0.15.0`; foundation audit `v0.17.0`; schema/persistence/builders `v0.22.1`, `v0.39.1`, `v0.140.0`–`v0.140.1`; engine `v0.60.0`–`v0.61.0`; downstream `v0.133.0`–`v0.138.0` |
