...
| exact interval and hard/statistical semantics | included/excluded/unbounded algebra without quantum adjustment; finite trusted estimates; containment claims do not imply source honesty; later confidence/error budgets extend the stable types | `v0.7.1`, consumers `v0.8.0`–`v0.15.0`; engine closure `v0.133.0`–`v0.136.0` |
...
| controlled hosted time-data updates | explicit provider, caller-serialized transaction, non-circular artifact trust, engine-owned opaque EOP/scale-offset admission proofs, authentication/authority separation, and later all-component concurrent publication | `v0.52.1`–`v0.52.3`, `v0.137.1`, `v0.148.0` |
...
| retained TLS/NTS state has correct lifetimes without live-clock churn | stable service context and ticket authorization separated from fresh per-handshake connection, per-connection exporter, and per-exporter NTS association generations | `v0.75.1`–`v0.75.2`, consumers `v0.77.0`–`v0.81.0`; audit `v0.162.0` |
