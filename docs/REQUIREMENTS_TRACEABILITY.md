...
| exact interval and hard/statistical semantics | included/excluded/unbounded algebra without quantum adjustment; finite trusted estimates; containment claims do not imply source honesty; bounded content-addressed conditions distinguish `All`, `Any`, conversion prerequisites, and threshold/fault rules; external identities resolve against exact content/registry; engine-verified root/derived claim derivations prevent unjustified narrowing; typed atom basis distinguishes measured/cryptographic evidence from authority assertions and configured adversary assumptions; snapshot-consistent runtime assessment and policy acceptance are required for current trusted authority and propagate invalidation | `v0.7.1`–`v0.7.3`, consumers `v0.8.0`–`v0.15.0`; schema/persistence `v0.22.1`, `v0.39.1`; engine `v0.60.0`–`v0.61.0`; downstream `v0.133.0`–`v0.138.0` |
...
| consistent concurrent reads | strict read-side exact-domain deadline enforcement, generation-consistent hosted publication, and explicit no_std concurrency profiles | `v0.137.0`–`v0.137.2` |
