...
| consistent concurrent reads | provider-owned conservative monotonic intervals; refresh uses a serialized pre-commit sample plus reviewed remaining-work bound through linearization, with typed unavailable state and no retained/new authority on failure; explicit linearization-time `observed_at`/`valid_until` authority versus WCET-capability-backed through-completion authority; generation-consistent publication and no_std profiles | primitive `v0.16.0`; traits/platform `v0.24.0`, `v0.30.0`, `v0.37.0`–`v0.38.2`; issuance `v0.60.1`; reads `v0.137.0`–`v0.138.0` |
