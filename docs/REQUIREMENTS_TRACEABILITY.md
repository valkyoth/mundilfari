...
| hard versus statistical uncertainty | stable source-neutral bounded interval and non-substitution types precede era/fraction/EOP; later confidence/model semantics and error budgets extend them | `v0.7.1`, `v0.14.0`–`v0.15.0`; engine closure `v0.133.0`–`v0.136.0` |
...
| controlled hosted time-data updates | explicit provider, caller-serialized transaction, non-circular artifact trust, opaque EOP/scale-offset admission proofs, authentication/authority separation, and later all-component concurrent publication | `v0.52.1`–`v0.52.3`, `v0.137.1`, `v0.148.0` |
...
| retained TLS/NTS state follows credential-context changes without live-clock churn | service/connection-bound immutable evidence plus stable policy/model/lifecycle identity and worst-case conservative monotonic retention horizon | `v0.75.1`, consumers `v0.77.0`–`v0.81.0`; audit `v0.162.0` |
