...
| exact interval and hard/statistical semantics | one early canonical identity profile binds all claim/recipe/condition/model/origin preimages and structurally handles collisions; included/excluded/unbounded algebra has no quantum adjustment; borrowed claims have generative lifetime-branded handles, while fallible owned claims canonically promote into bounded frozen storage without self-reference/lifetime forgery; nonwrapping generations, lease traversal, explicit equality, and no arena-dependent infallible semantic `Eq`/`Hash`; engine proofs/tokens become source-arena-independent but retain every invalidation generation; external/persisted recipes remain unverified records | identity `v0.6.1`; claims/recipes/ownership `v0.7.1`–`v0.15.0`; foundation audit `v0.17.0`; schema/persistence/builders `v0.22.1`, `v0.39.1`, `v0.140.0`–`v0.140.1`; engine `v0.60.0`–`v0.61.0`; downstream `v0.133.0`–`v0.144.0` |
