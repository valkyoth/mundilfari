...
| exact interval and hard/statistical semantics | included/excluded/unbounded algebra without quantum adjustment; finite trusted estimates; containment claims do not imply source honesty; immutable content-addressed assumptions compose without loss/substitution and fail on incompatibility/capacity; later confidence/error budgets extend the stable types | `v0.7.1`–`v0.7.2`, consumers `v0.8.0`–`v0.15.0`; engine closure `v0.60.0`–`v0.61.0`, `v0.133.0`–`v0.136.0` |
...
| controlled hosted time-data updates | explicit provider, caller-serialized transaction, non-circular artifact trust, untrusted platform retrieval claims separated from engine-verified evidence and configured authority, opaque EOP/scale-offset admission proofs, and later all-component concurrent publication | `v0.52.1`–`v0.52.3`, `v0.137.1`, `v0.148.0` |
