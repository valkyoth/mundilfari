...
| exact interval and hard/statistical semantics | included/excluded/unbounded algebra without quantum adjustment; finite trusted estimates; containment claims do not imply source honesty; bounded content-addressed conditions distinguish `All`, `Any`, conversion prerequisites, and threshold/fault rules; every early root/transformation preserves a bounded non-authoritative claim recipe and external/persisted recipes remain unverified records; engine verification prevents unjustified narrowing; structured atom basis keeps origin, integrity, authority, and transitive derivation axes distinct; snapshot-consistent runtime assessment and policy acceptance are required for current trusted authority and propagate invalidation | `v0.7.1`–`v0.15.0`; schema/persistence `v0.22.1`, `v0.39.1`, `v0.140.1`; engine `v0.60.0`–`v0.61.0`; downstream `v0.133.0`–`v0.138.0` |
...
| consistent concurrent reads | conservative upper-edge monotonic deadline checks including resolution/latency/rate/completion uncertainty, generation-consistent hosted publication, and explicit no_std concurrency profiles | primitive `v0.16.0`; issuance `v0.60.1`; reads `v0.137.0`–`v0.137.2` |
