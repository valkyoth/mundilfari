...
| implementation stops remain small enough for one review and pentest pass | machine validator rejects more than 16 top-level deliverables or 180 milestone lines; dense independently reviewable responsibilities still require an earlier split; no gate exemption | every milestone; current decompositions `v0.60.1`–`v0.60.7`, `v0.137.1`–`v0.137.7` |
| exact interval and hard/statistical semantics | exact borrowed/owned/view types with no third public generic claim type; one canonical identity profile; borrowed generative handles; fallible single/multi-root promotion into bounded frozen shared DAGs with atomic failure, per-root/unique-total accounting, immutable retention and bounded new-owner compaction; nonwrapping generations, lease traversal, explicit equality, and no arena-dependent infallible semantic `Eq`/`Hash`; engine proofs/tokens become source-arena-independent through one kind-safe checked engine handle and semantic views; canonical-order batch verification uses one bounded snapshot, shared-node failure fan-out, root-local evidence isolation, no authoritative global-abort prefix, complete accounting, processed-only complete member status, abort-only unprocessed diagnostics, and a typed complete-membership witness; original configured membership remains distinct from current accepted-bound contributors, failed members cannot vote or reduce thresholds; non-authoritative batch admission, exact-support consensus, and published authority have distinct identities and one-domain conservative validity with admitted-correlation translation or rejection; refresh reports retained/invalidated/absent prior state through a fixed-size tagged measured/unavailable observation using portable version-reserved linearization or optional commit-covered evidence, neither granting through-receipt authority; external/persisted recipes remain unverified records | identity `v0.6.1`; claims/recipes/ownership `v0.7.1`–`v0.15.0`; foundation audit `v0.17.0`; schema/persistence/builders `v0.22.1`, `v0.39.1`, `v0.140.0`–`v0.140.1`; engine `v0.60.0`–`v0.61.0`; downstream/publication `v0.133.0`–`v0.144.0`; product/resource/fault/API gates `v0.148.0`, `v0.160.0`, `v0.163.0`, `v0.166.0` |
...
| leap evidence cannot unilaterally change UTC | source-neutral representation; pure isolated candidate transaction; evidence lifecycle; opaque engine admission handoff; atomic precommit generation/expiry recheck and consistent publication | `v0.12.0`–`v0.12.1`, `v0.15.2`, `v0.61.1`, `v0.137.3`; audit `v0.148.0` |
| monotonic values are domain typed | suspend/rate/scope/process/machine/namespace/generation identity; directed untrusted correlation candidates are structurally separate from opaque engine-admitted correlations; offset/rate/drift use canonical hard-bound derivation recipes and conditions, immutable paired capture anchors, outward uncertainty growth, and independently sourced endpoint deadlines; engine verifies derivations and snapshot-consistently assesses conditions/support before admission, and provider registration alone is not evidence; initial proof admission rejects every transitive current/stale/replaced/historical admitted-correlation recipe/condition/assessment/support dependency, making numerical support acyclic by construction and refusing two-node/longer/replacement/restored-reference cycles; direct translation rounds outward and maps deadlines to the target earliest edge; provider/domain/evidence/condition/anchor/endpoint/lifecycle change withdraws dependents, while circular validity and reverse/implicit composition are refused | kernel `v0.16.0`; foundation audit `v0.17.0`; provider traits/platform candidates `v0.24.0`, `v0.30.0`, `v0.37.0`–`v0.38.2`; engine admission `v0.60.2`; consumers `v0.61.0`, `v0.133.0`, `v0.137.1`; schema `v0.140.1`; final gates `v0.148.0`, `v0.160.0`–`v0.161.0`, `v0.163.0`, `v0.166.0` |
...
| complete platform foundations | provider-owned monotonic read intervals/capabilities and untrusted directed correlation candidates carrying proof recipes/conditions and exact capture anchors, hosted clocks, PHC, RTC, architectural/embedded counters, MMIO, GPIO, frequency capture, actuators, namespace identity; platform has no correlation-admission constructor and registration is not numerical evidence | traits `v0.24.0`; adapters `v0.30.0`, `v0.37.0`–`v0.40.0`; `v0.161.0` |
...
| consensus configuration identity | atomic policy/membership generations carried into results/proposals; original `n` is preserved across processed member failure, only current accepted bounds contribute, abort/unprocessed diagnostics cannot enter quorum, and removal requires a fully reassessed new generation; every consensus binds its exact used `ProofSupportSet`, one monotonic authority domain, conservative minimum validity, and admitted correlation derivations/condition assessments/structured support/capture anchors/independent endpoint validity, so loss of any used dependency invalidates the old identity even when another quorum is possible; tagged refresh disposition uses measured coverage or explicit unavailability | `v0.60.4`–`v0.61.0`, `v0.133.0`, concurrent refresh/publication `v0.137.1`–`v0.137.2` |
...
| consistent concurrent reads | provider-owned conservative monotonic intervals; portable `LinearizationRefresh` uses engine-only RAII `RefreshReservationGuard` ownership binding process/session/machine generations, a captured invalidation watermark advanced before every dependency mutation, nonwrapping fencing, sample-as-logical-point, and an in-place `Live`→`Tombstone` cleanup that consumes no separate capacity and is bounded/allocation-free/callback-free/nonblocking/non-panicking during drop/unwind/panic/cancel; superseded cleanup is a successful fenced no-op, impossible transitions fault the engine and disable install, full slots reject acquisition but never cleanup, readers have bounded results, delayed-install ordering, and strict-reader revalidation without WCET; supersession requires a tombstoned or execution-invalidated owner and never timeout alone, stale writers return `SupersededNoInstall`, restart invalidates inherited reservations, and tombstone reclamation waits behind a bounded reader generation floor while reclamation/fence exhaustion fails closed; async performs all pending/fallible/external work before guard acquisition and returns `Ready` in that poll; optional `CommitCoveredRefresh` adds reviewed remaining-work coverage through physical commit; typed unavailable state cannot retain/install new admission or authority while ordered invalidation tombstones remain permitted; explicit strict-read linearization authority versus WCET-capability-backed through-completion authority | primitive `v0.16.0`; lifecycle `v0.23.1`; traits/platform `v0.24.0`, `v0.30.0`, `v0.37.0`–`v0.38.2`; engine refresh `v0.60.5`; publication `v0.137.1`–`v0.137.5`; no_std `v0.137.6`; async/API `v0.138.0`–`v0.140.1`; final gates `v0.148.0`, `v0.160.0`, `v0.163.0`, `v0.166.0` |
| honest monotonic application-clock recovery | truth bounds may revise, preferred projection cannot retain false synchronized status | `v0.137.7`, `v0.166.0` |
