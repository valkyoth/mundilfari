...
| monotonic values are domain typed | suspend/rate/scope/process/machine/namespace/generation identity; directed untrusted correlation candidates are structurally separate from opaque engine-admitted correlations; direct translation rounds outward and maps deadlines to the target earliest edge; provider/domain/lifecycle change withdraws dependents, and reverse/implicit composition is refused | kernel `v0.16.0`; provider traits/platform candidates `v0.24.0`, `v0.30.0`, `v0.37.0`–`v0.38.2`; engine admission `v0.60.1`; consumers `v0.61.0`, `v0.133.0`, `v0.137.1` |
...
| complete platform foundations | provider-owned monotonic read intervals/capabilities and untrusted directed correlation candidates, hosted clocks, PHC, RTC, architectural/embedded counters, MMIO, GPIO, frequency capture, actuators, namespace identity; platform has no correlation-admission constructor | traits `v0.24.0`; adapters `v0.30.0`, `v0.37.0`–`v0.40.0`; `v0.161.0` |
...
| consensus configuration identity | atomic policy/membership generations carried into results/proposals; original `n` is preserved across processed member failure, only current accepted bounds contribute, abort/unprocessed diagnostics cannot enter quorum, and removal requires a fully reassessed new generation; every consensus binds its exact used `ProofSupportSet`, one monotonic authority domain, conservative minimum validity, and admitted correlation dependencies, so loss of used support invalidates the old identity even when another quorum is possible; tagged refresh disposition uses measured coverage or explicit unavailability | `v0.60.1`–`v0.61.0`, `v0.133.0`, concurrent refresh/publication `v0.137.1` |
...
| consistent concurrent reads | provider-owned conservative monotonic intervals; portable `LinearizationRefresh` uses a nonwrapping reservation/version, sample-as-logical-point, bounded reader retry/failure, invalidation watermark, delayed-install ordering, and strict-reader revalidation without WCET; optional `CommitCoveredRefresh` adds reviewed remaining-work coverage through physical commit; typed unavailable state cannot retain/install new admission or authority while ordered invalidation tombstones remain permitted; explicit strict-read linearization authority versus WCET-capability-backed through-completion authority | primitive `v0.16.0`; traits/platform `v0.24.0`, `v0.30.0`, `v0.37.0`–`v0.38.2`; issuance `v0.60.1`; publication/no_std/API `v0.137.1`–`v0.138.0` |
