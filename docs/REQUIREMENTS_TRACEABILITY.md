...
| monotonic values are domain typed | suspend/rate/scope/process/machine/namespace/generation identity; directed untrusted correlation candidates are structurally separate from opaque engine-admitted correlations; offset/rate/drift use canonical hard-bound derivation recipes and conditions, immutable paired capture anchors, outward uncertainty growth, and independently sourced endpoint deadlines; engine verifies derivations and snapshot-consistently assesses conditions/support before admission, and provider registration alone is not evidence; direct translation rounds outward and maps deadlines to the target earliest edge; provider/domain/evidence/condition/anchor/endpoint/lifecycle change withdraws dependents, while circular validity and reverse/implicit composition are refused | kernel `v0.16.0`; foundation audit `v0.17.0`; provider traits/platform candidates `v0.24.0`, `v0.30.0`, `v0.37.0`–`v0.38.2`; engine admission `v0.60.1`; consumers `v0.61.0`, `v0.133.0`, `v0.137.1`; final gates `v0.148.0`, `v0.160.0`–`v0.161.0`, `v0.163.0`, `v0.166.0` |
